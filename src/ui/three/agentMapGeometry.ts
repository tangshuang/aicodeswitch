...
/**
 * 时间（会话年龄，天）→ 3D 世界径向半径：阶梯式分段映射。
 * 临界值 1 / 7 / 30 / 365 天，段长比例 20% / 30% / 30% / 20%；其中 0–1 天段 = 早期版本的 2 倍（32），
 * 其余按比例计算。超过 365 天的节点饱和在最外（365 日环）外围。
 *   0–1d → 32（20%），1–7d → 48（30%），7–30d → 48（30%），30–365d → 32（20%）；365d 环 = 160。
 */
const TIME_TIERS = [0, 1, 7, 30, 365]; // 天
const TIME_BANDS = [32, 48, 48, 32];   // 各段径向长度：[0–1d, 1–7d, 7–30d, 30–365d]
const TIME_RADII: number[] = (() => {
  const arr = [0];
  let r = 0;
  for (const b of TIME_BANDS) { r += b; arr.push(r); }
  return arr; // [0, 32, 80, 128, 160]
})();
