...
- **Key Files**: `memory.ts`(目录/记忆/会话索引+指针/原子写/遗留迁移读写 + `capCli`/`LeaderCliEntry`/`CLI_PERSIST_BUDGET`) / `prompt.ts`(系统提示+上下文拼装) / `runner.ts`(流式 claude spawn + stream-json 增量解析 + 捕获 `session_id` + `onCli` 原始 stdout/stderr 通道) / `manager.ts`(`LeaderManager` 多会话 + 单活跃流 + artifacts 采集/清理 + Codex 快照差分 + 每轮 `cliEntries` 累加/`capCli` 持久化) / `mcp-server.ts`(独立进程入口) / `routes.ts` / `index.ts`
...
- **前端**: `HomePage.tsx` 为 Codex 风格聊天窗（左侧会话列表面板与对话窗口并列、默认收起/滑动展开；消息区 user/assistant 气泡 + ReactMarkdown + 工具调用折叠 chip，底部输入框，`fetch().body.getReader()` 流式逐字渲染）；菜单名「首页」。**每条 assistant 消息**带 meta 行：agent 类型徽标（Claude Code/Codex，按消息记录的 `leaderTool`，老消息回退会话级）+ 时间 + **CLI 开关**（点击展开终端风格区域，实时刷新该轮 spawn 的 stdout/stderr，完成后可查看完整 CLI 上下文；流式中脉冲圆点示「实时」，出错强制展开）。无独立调试面板
- **CLI 输出通道（重构）**：原始 stdout/stderr 经 `onCli`（`{s:'stdout'|'stderr';t:string}`）流出，SSE 事件类型 `cli`（不再有 `debug`）。runner 仅从 4 个原始站点喂数据（Claude 不再做 raw+重序列化双发）；其余生命周期诊断改服务端 `console.error`。`manager.sendMessage` 每轮累加 `cliEntries` 并在 `onDone`/`onError` 时经 `capCli()`（head+tail 截断，`CLI_PERSIST_BUDGET=64KB`，实时流不截断）持久化到 `ConversationMessage.cli`，刷新/重启后仍可回看
...
- **已知约束**: Leader 的 Claude 路径由 **Claude Agent SDK**（`@anthropic-ai/claude-agent-sdk`）的 `query()` 驱动（不再手动 spawn CLI）。SDK 自管子进程/协议/流式，规避了旧实现手动 spawn + stdin 在 server 运行时下 claude 读完全部 prompt 后 exit 1 零输出的死局。事件映射：`system/init`→onSessionId、`stream_event`(content_block_delta/text_delta)→onText 逐字、`assistant`→onTool、`result`→终态文本、SDK `stderr` 回调→onCli。经 `settingSources:['user','project']` 读 `~/.claude/settings.json` 的 `ANTHROPIC_BASE_URL` 走代理；MCP 经 `mcpServers` 内联（`strictMcpConfig`）；权限经 `canUseTool`；超时/取消经 `abortController`。Codex 路径仍用 `codex exec` CLI spawn，待 Codex SDK 代理路由验证后迁移；MCP 走 HTTP 调本机，若开启 AUTH 需 `ATO_TOKEN`；单活跃会话避免并发冲突
...
- **权限裁决（PermissionJudge，`leader/permission.ts`）**：经 Claude Agent SDK 的 `options.canUseTool` 回调接入——SDK 每次工具调用时回调 `(toolName,input)=>PermissionResult`，runner 内调 `PermissionJudge.evaluate`。裁决：先硬规则（deny/allow 正则）→ 否则 LLM 危险度分析（经本机代理 `/v1/messages` 打一次上游，不走 CLI、不递归）→ low 放行 / high 拒绝（deny message 含建议喂回 claude 让其 adapt）/ medium 按配置自动或上抛人类。上抛人类经 pending 队列 + `GET /permissions/stream` SSE 推前端，UI 卡片放行/拒绝 → `/permissions/:id/resolve`。配置在 `ato-leader/config.json` 的 `permission`（enabled/allowPatterns/denyPatterns/humanGateMedium/humanGateHigh）。`permission.enabled` 默认 opt-in（关闭时 leader 以 `permissionMode:'bypassPermissions'`+`allowDangerouslySkipPermissions:true` 自由运行；开启时 `permissionMode:'default'`+canUseTool 逐工具裁决）。旧的 `--permission-prompt-tool` 注入、MCP `permission_request` 工具、`~/.claude.json` 预检均已废弃移除（claude 2.1.37 该 flag 启动期拿不到 MCP 工具，实测 exit 1）。Codex 路径不接权限
